    ...
    <v-card color="">
            ...
            <v-card color="black" dark>
              ...
              <v-card-actions>
                <div class="container">
                  <v-btn color="green" icon large>
                    <div class="d-flex">
                      <h2 style="margin-start: 38px; font-size: 1.3rem; color: green">
                        {{ numeral(totalSale).format("0,0.0") }}
                      </h2>
                    </div>
                  </v-btn>
                  <h4>Total Sale</h4>
      ...
      >
        ...
        <template v-slot:item.actions="{ item }">
          <v-btn icon :to="`/1005/admin/viewOrder/${item.id}`"
...
export default {
  ...
  methods: {
    ...
    FetchTotalSale22(val) {
      this.totalSale = 0;
      let vall = this.moment(new Date(val)).format("MMM YYYY");
      const db = this.$fire.firestore;
      db.collection("Order_request")
        .where("cart", "==", false)
        .where("month", "==", vall)
